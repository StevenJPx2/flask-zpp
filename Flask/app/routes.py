...
def user(username):
    ...
    posts = user.posts.order_by(Post.timestamp.desc()).paginate(
        pages, app.config['POSTS_PER_PAGE'], False
    )
    ...
    return render_template('user.html', title=f"{user.username}'s page", 
...
def edit_profile():
    ...
    return render_template("edit_profile.html", title=f"Edit Profile - {current_user.username}", form=form)
...
def unfollow(username):
    ...
    if user is None:
        ...
        db.session.commit()
        flash(f'You have now unfollowed {user}!')
        return(redirect(url_for('user', username=username)))
